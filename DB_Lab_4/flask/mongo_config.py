...
from SQLA_config import Students, Locations, EO, Tests, Znorecords
...
psql_engine = create_engine("postgresql+psycopg2://postgres:root@db/postgresdb")
...
znorecords_collection = db.znorecords
...
class Config:
    ...
    def __init__(self):
        if locations_collection.count_documents({}) == 0:
            ...
            print("\nMigrating Zno Records to MongoDB...")
            self.createZnorecordsCollection()
    ...
    def createZnorecordsCollection(self):
        znorecords_collection = db.znorecords
        document_count = znorecords_collection.count_documents({})
        if document_count == 0:
            for record in session.query(Znorecords):
                zno_data = {}
                for column in Znorecords.columns:
                    column_name = column.name
                    column_value = getattr(record, column_name)
                    if isinstance(column_value, Decimal):
                        column_value = float(column_value)
                    zno_data[column_name] = column_value
                znorecords_collection.insert_one(zno_data)
    ...
    def createStudent(self, student_id, year_of_passing, outid, birth, sextypename, location_id, eo_id,
                      tests_results_id):
        student_data = {
    ...
    def updateStudent(self, student_id, year_of_passing, outid, birth, sextypename, location_id, eo_id,
                      tests_results_id):
    ...
    def fetchGrade(self, year, regname, subject, function):
        ...
        pipeline = [
            {"$group": {
                "_id": {"year": "$year", "regname": "$regname"},
                "result": {f"${function}": {f"${subject}": "$$CURRENT"}}
            }},
            {"$project": {"result": 1}}
        ]

        results = list(znorecords_collection.aggregate(pipeline))
        if results:
            result = results[0]["result"][f"${function}"][f"${subject}"]
        else:
            result = None
    ...
    def subjectDict(self):
        return {"Українська мова та література": "umlball100",
                "Українська мова": "ukrball100",
                "Історія України": "HistBall100", "Математика": "mathball100",
                "Фізика": "physball100", "Хімія": "chemball100",
                "Географія": "geoball100",
                "Англійська мова": "engball100", "Французька мова": "frball100",
                "Німецька мова": "deuball100", "Іспанська мова": "spball100"}
